...
class VCG:
    ...
    def compute(slot_clicks, reserve, bids):
        ...
        def total_payment(k):
            ...
            if k > n:
                return 0
            elif k == n-1: # 0-indexed? k is 0-indexed
                if len(valid_bids) <= n: # bids above reserve price is less than allocation number
                    return c[k] * reserve # some allocated agents pay reserve; WILL MORE THAN 1 PAY RESERVE??
                return c[k] * valid_bids[k+1][1]
            elif k < n-1 and k >= 0:
                return (c[k] - c[k+1])*valid_bids[k+1][1] + total_payment(k+1)
            else:
                return IndexError("Invalid slot input k")
